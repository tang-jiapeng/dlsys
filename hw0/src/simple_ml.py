...
def parse_mnist(image_filename, label_filename):
    ...
    with gzip.open(image_filename, "rb") as f:
        data = f.read()
        _, num_images, rows, cols = struct.unpack_from(">IIII", data, 0)
        offset = struct.calcsize(">IIII")
        image_data = np.frombuffer(data, dtype=np.uint8, offset=offset)
        X = image_data.reshape(num_images, rows * cols).astype(np.float32)
        X /= 255.0

    with gzip.open(label_filename, "rb") as f:
        data = f.read()
        _, num_labels = struct.unpack_from(">II", data, 0)
        offset = struct.calcsize(">II")
        label_data = np.frombuffer(data, dtype=np.uint8, offset=offset)
...
def softmax_loss(Z, y):
    ...
    loss = -np.log(softmax_probs[np.arange(n), y])
...
def softmax_regression_epoch(X, y, theta, lr=0.1, batch=100):
    ...
    for start in range(0, num_examples, batch):
        ...
        gradient = np.dot(X_batch.T, (softmax_probs - I_y)) / len(y_batch)
        theta -= lr * gradient
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for start in range(0, num_examples, batch):
        ...
        G2 = (softmax_probs - I_y) / (end - start)
        G1 = np.dot(G2, W2.T) * (Z1 > 0)

        W2 -= lr * np.dot(Z1.T, G2)
        W1 -= lr * np.dot(X_batch.T, G1)
    ### END YOUR CODE
